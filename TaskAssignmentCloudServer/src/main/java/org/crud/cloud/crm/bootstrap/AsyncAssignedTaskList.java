...
@ServiceInfo(uri="/async/assignedtasklist")
public class AsyncAssignedTaskList implements MobileServiceBean
{
	private static Logger log = Logger.getLogger(AsyncAssignedTaskList.class);
	//ResponseDS ds;
	TaskDS ds;
	...
	public void start()
	{
		log.info(TAG);
		log.info("--------------------------------------------------------------------------");
		log.info("AsyncAssignedTaskList : was successfully started....");
		log.info("--------------------------------------------------------------------------");
	}
	...
	{	
		...
		try
		{
			...
			List<Task> taskList = this.ds.readByTaskByAssigneeDeviceId(device.getIdentifier());
