...
EditText taskTitle, taskDesc, taskCreater, response, responder;
	...
	void initializeControls()
	{
		...
		sendResponse.setVisibility(View.VISIBLE);
		sendResponse.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				TaskResponseObject ro = new TaskResponseObject();
				ro.setResponseDesc(response.getText().toString());
				ro.setTaskServerId(to.getServerTaskId());
					
				CreateResponseOverServerAsyncOM c = new CreateResponseOverServerAsyncOM(AssignedTaskFullDetailsActivity.this, handlerCreateResponse, ro);
				c.execute();
	...
	public void postCheckoutProcessing()
	{
		...
		to = AssignedTaskListingActivity.listTaskObjects.get(taskItemPostion);
	...
	void initializeHandler()
	{
		...
		//		super.handleMessage(msg);
				...
				if(responseDsc == null || responseDsc.equals(""))
				{
					sendResponse.setVisibility(View.VISIBLE);
		...
		handlerCreateResponse = new Handler()
		{
			...
			public void handleMessage(Message msg) {
				...
				if(msg.what == 1)
				{
					Toast.makeText(AssignedTaskFullDetailsActivity.this, "Response has been saved",Toast.LENGTH_LONG).show();
					sendResponse.setVisibility(View.GONE);
				}
